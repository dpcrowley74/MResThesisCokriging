...
%
% Outputs:
%       model - An object called model containing all the data for predictions
%       f - a constant indicating if any of the Cjolesky factorization was
%       unsuccesful
...
% a small number to reduce ill conditioning of the matrix
...
if f == 0
    ...
    if nargin > 4
        ...
        % Cholesky factorization
        [Ud,f] = chol(PsidXe);
        % Check if the matrix was propelry factorized
        if f == 0
            % Difference vector
            d = Ye - rho.*Yc(nc+1:n);
            ...
            % Cholesky factorization
            [U,f] = chol(C);
            %check if the matrix was properly factorized
            if f== 0
